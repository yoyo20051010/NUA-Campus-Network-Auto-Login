...
学校是**整点断网**（默认周一~周五 00:00）。如果等它断了再切，正在跑的连接会直接断——表现出来就是"每天零点卡一下"。

`pre_switch_minutes` 让脚本**提前几分钟**就把这条线摘出备用池：
...
```
23:55  脚本提前执行 mwan3 ifdown wan
       ├─ 新连接立刻走备用线（教师线）
       └─ 已经在跑的连接还留在原线上，能正常跑完（线还没断）
00:00  学校断网 —— 这时这条线上基本已经没有流量了，用户察觉不到
06:00  夜间时段结束，脚本重新登录，成功后 mwan3 ifup 切回来（在健康线路上切换）
```

**关键点**：提前切换是在**线路还健康的时候**做的，所以现有连接不受影响；等真正断网时，那条线上已经没什么流量了。

设 `0` 可以关掉这个行为。

### 内存自保

实测这个脚本的常驻内存会缓慢增长（约 1MB/小时）。路由器内存本来就小，涨到一定程度会触发内核 OOM——之前发生过一次，脚本被 OOM 杀掉，而且前后 mwan3 的跟踪进程还卡死过。
...
占用超过阈值就**主动退出**，让 procd 立刻把服务拉起来。比被 OOM 杀掉可控得多：日志里有明确记录，中断只有一两秒，起来后立刻恢复检查。设 `0` = 不检查。
